...
export default function Home() {
  ...
  // Banner state
  const [banner1, setBanner1] = useState<{ displayImage: string; title?: string; subtitle?: string; ctaText?: string; ctaLink?: string } | null>(null);
  const [banner2, setBanner2] = useState<{ displayImage: string; title?: string; subtitle?: string; ctaText?: string; ctaLink?: string } | null>(null);
  ...
  useEffect(() => {
    const fetchBanners = async () => {
      try {
        ...
        if (response.success) {
          const banner1Data = response.data.find((b: any) => b.section === 'BANNER_1' && b.isActive);
          const banner2Data = response.data.find((b: any) => b.section === 'BANNER_2' && b.isActive);
          
          if (banner1Data) {
            setBanner1({
              displayImage: banner1Data.displayImage,
              title: banner1Data.title,
              subtitle: banner1Data.subtitle,
              ctaText: banner1Data.ctaText,
              ctaLink: banner1Data.ctaLink,
            });
          }
          
          if (banner2Data) {
            setBanner2({
              displayImage: banner2Data.displayImage,
              title: banner2Data.title,
              subtitle: banner2Data.subtitle,
              ctaText: banner2Data.ctaText,
              ctaLink: banner2Data.ctaLink,
            });
          }
        }
      } catch (error) {
  ...
  return (
    <div className="overflow-hidden">
      ...
      {!bannersLoading && banner1 && (
        <section className="w-full relative">
          <img 
            src={banner1.displayImage || '/images/banner-1.jpg'} 
            alt={banner1.title || 'African Fashion Collection'} 
            className="w-full h-48 md:h-64 lg:h-80 object-cover"
          />
          {(banner1.title || banner1.subtitle) && (
            <div className="absolute inset-0 bg-gradient-to-r from-black/60 via-black/30 to-transparent flex items-center">
              <div className="px-8 md:px-12 lg:px-16">
                {banner1.title && (
                  <h2 className="text-white text-2xl md:text-3xl lg:text-4xl font-bold mb-2">{banner1.title}</h2>
                )}
                {banner1.subtitle && (
                  <p className="text-white/80 text-base md:text-lg mb-4">{banner1.subtitle}</p>
                )}
                {banner1.ctaText && banner1.ctaLink && (
                  <Link 
                    to={banner1.ctaLink}
                    className="inline-flex items-center gap-2 bg-coral-500 text-white px-6 py-3 rounded-lg font-medium hover:bg-coral-600 transition-colors"
                  >
                    {banner1.ctaText} <ArrowRight className="w-4 h-4" />
                  </Link>
                )}
              </div>
            </div>
      ...
      {!bannersLoading && banner2 && (
        <section className="w-full relative">
          <img 
            src={banner2.displayImage || '/images/banner-2.jpg'} 
            alt={banner2.title || 'Become a Seller'} 
            className="w-full h-48 md:h-64 lg:h-80 object-cover"
          />
          {(banner2.title || banner2.subtitle) && (
            <div className="absolute inset-0 bg-gradient-to-r from-black/60 via-black/30 to-transparent flex items-center">
              <div className="px-8 md:px-12 lg:px-16">
                {banner2.title && (
                  <h2 className="text-white text-2xl md:text-3xl lg:text-4xl font-bold mb-2">{banner2.title}</h2>
                )}
                {banner2.subtitle && (
                  <p className="text-white/80 text-base md:text-lg mb-4">{banner2.subtitle}</p>
                )}
                {banner2.ctaText && banner2.ctaLink && (
                  <Link 
                    to={banner2.ctaLink}
                    className="inline-flex items-center gap-2 bg-coral-500 text-white px-6 py-3 rounded-lg font-medium hover:bg-coral-600 transition-colors"
                  >
                    {banner2.ctaText} <ArrowRight className="w-4 h-4" />
                  </Link>
                )}
              </div>
            </div>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="lg:pl-8">
              ...
              <p className="text-white/80 mb-6 leading-relaxed">
                For centuries, African textiles have told stories of culture, identity, and tradition. From the vibrant Kente cloths of Ghana to the intricate Adinkra symbols, each fabric carries the legacy of generations of skilled artisans.
              </p>

              <p className="text-white/80 mb-8 leading-relaxed">
                Our marketplace connects you directly with these master craftspeople, preserving traditional techniques while bringing African fashion to the global stage. Every purchase supports local communities and keeps these precious traditions alive.
              </p>
              ...
              <div className="grid grid-cols-3 gap-6">
                ...
                <div className="text-center">
                  ...
                  <div className="text-white/60 text-sm">Designs</div>
                </div>
                <div className="text-center">
                  <div className="text-4xl font-bold text-coral-400 mb-2">2K+</div>
